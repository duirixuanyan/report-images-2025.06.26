...
! includes absorption via standard absorption lengths
    USE DFLIB
    IMPLICIT NONE
    INTEGER :: i,N,in,idum=12345,numsmpl,numsmplhf,ns1=1
    REAL    :: L,kap,sig,beta,QT,DQ,x0,dx,dy,R,qd(50),thd,psi,xi,yi,zi
    REAL    :: sx,sy,sz,qdav,stddev,xe,ye,ze,sinth,ri,re,labsi
    REAL    :: labs,lscat,lend,szdet,Ibndl,z1,z2,lpart,a,b,c,d,Qadj,var,stddevmax
    REAL, PARAMETER :: pi=3.1415927
    REAL, EXTERNAL :: RAN1
    open(unit=8,file='revmck1.dat',status='unknown')
    write(8,1)
    write(*,1)
...
! initial point is inside
             IF(re<R) THEN
                 Ibndl=Ibndl+(exp(-beta*zi)-exp(-beta*ze))/sz
             ELSE
                 lpart=-(b-sqrt(d))/c     ! distance to collimated beam exit point
                 z2=zi+lpart*sz
                 Ibndl=Ibndl+(exp(-beta*zi)-exp(-beta*z2))/sz
             ENDIF
          ELSE
! initial point is outside
             IF(re<R) THEN
                 ...
                 z1=zi+lpart*sz
                 Ibndl=Ibndl+(exp(-beta*z1)-exp(-beta*ze))/sz
             ELSE
                 ...
                 z2=zi+lpart*sz
                 Ibndl=Ibndl+(exp(-beta*z1)-exp(-beta*z2))/sz
             ENDIF
          ENDIF
! bundle leaves slab: tally up and go to next bundle
 25       IF(ze<0.00001*L .or. ze>0.99999*L .or. labsi>labs) THEN
             qd(i)=qd(i)+DQ*Ibndl
             CYCLE
          ENDIF
          xi=xe
