//      PROGRAM P1_2D
// **** P-1 APPROXIMATION FOR 2D RECTANGLE OR CYLINDER, FRONT-END FOR P1SUB ****
// HERE IMPLEMENTED FOR A CYLINDER OF RADIUS R, LENGTH L, AT ZERO TEMPERATURE
// BOUNDED BY BLACK COLD LINER AND BLACK COLD FACE AT L, BOUNDED BY GRAY
// FACE AT 0 WITH TEMPERATURE TX(1)
// ALLOWS FOR ABSORPTION (KT) AND LINEAR-ANISOTROPIC SCATTERING (ST, A1), HERE USING
...
int main()
{
      ...
      int I,J,LU,JPR;
		double QR[3][II+1],QX[3][JJ+1],EPSR[3],EPSX[3],TR[3],TX[3],TM[II+1][JJ+1],G[II+1][JJ+1],L,R;
      double SIG,A1[II+1][JJ+1],KT[II+1][JJ+1],ST[II+1][JJ+1],BT[II+1][JJ+1],SS[II+1][JJ+1],SR[3][II+1],SX[3][JJ+1];
      
      //output file
      ofstream outfile("P1_2Dc.dat", ios::out);
      
      LU=9;
		SIG=5.670E-12;     // W/cm2K4
      TX[1]=2000.E0;     // ASSUME UNIFORM TEMPERATURE FOR ALL NODES
      TX[2]=0.E0;        // ASSUME UNIFORM TEMPERATURE FOR ALL NODES
      TR[1]=0.E0;        // ASSUME UNIFORM TEMPERATURE FOR ALL NODES
      TR[2]=0.E0;        // ASSUME UNIFORM TEMPERATURE FOR ALL NODES
      for (I=1;I<=II;I=I+1)
       {for (J=1;J<=JJ;J=J+1)
         {TM[I][J]=0;}}    // ASSUME UNIFORM TEMPERATURE FOR ALL NODES
		R=10.E0;             // cm
		L=2.E0;              // cm
      for (I=1;I<=2;I=I+1) EPSR[I]=1.E0;       // BLACK LINER
      EPSX[1]=.5E0;      // BASE AT X=0
      EPSX[2]=1.E0;      // TOP AT X=L
      for (I=1;I<=II;I=I+1)
       {for (J=1;J<=JJ;J=J+1)
			{KT[I][J]=1.E0;}}       //cm-1; CONST THROUGHOUT
      for (I=1;I<=II;I=I+1)
       {for (J=1;J<=JJ;J=J+1)
			{ST[I][J]=1.E0;}}       //cm-1; CONST THROUGHOUT
      for (I=1;I<=II;I=I+1)
       {for (J=1;J<=JJ;J=J+1)
         {A1[I][J]=0.E0;}}       // CONST THROUGHOUT      

      JPR=(JJ-1)/11+1;   // PRINT SPACING FOR Y/r-NODES  
      
// ESTABLISHMENT OF INTERNAL SOURCES IF PRESENT 

      for (I=1;I<=II;I=I+1)
        {
        for (J=1;J<=JJ;J=J+1)
      {
          SS[I][J]=4.E0*SIG*pow((TM[I][J]),4.);
      }}

// ESTABLISHMENT OF BOUNDARY SOURCES IF PRESENT

      for (I=1;I<=II;I=I+1)
        {
        SR[2][I]=4.E0*SIG*pow((TR[2]),4.);
        SR[1][I]=4.E0*SIG*pow((TR[1]),4.);
    }
      for (J=1;J<=JJ;J=J+1)
        {
        SX[1][J]=4.E0*SIG*pow((TX[1]),4.);
        SX[2][J]=4.E0*SIG*pow((TX[2]),4.);
        } 


// CALCULATION OF INCIDENT RADIATION FIELD

		P1SOR(L,R,EPSX,EPSR,SX,SR,KT,ST,A1,SS,G,QX,QR);

// CALCULATE TEMPERATURE IF RADIATIVE EQUILIBRIUM (IRE=1)
		...
		if(IRE==1)
			...
			for (I=1;I<=II;I=I+1)
				...
				for (J=1;J<=JJ;J=J+1)
					{
					TM[I][J]=pow(((SS[I][J]/(KT[I][J]+1.E-10)+G[I][J])/(4.E0*SIG)),.25);
					}
				}
			}
...
// OUTPUT
      outfile<<setiosflags( ios::fixed | ios::showpoint )<<setprecision(2);
		if(KK==0)
			{
			 outfile<<"          GENERAL DATA"<<endl;
			 outfile<<"          ************"<<endl;
			 outfile<<" "<<endl;
			 outfile<<"          RECTANGLE HEIGHT (y-DIR):  "<<setw(9)<<R<<"CM"<<endl;
			 outfile<<"          RECTANGLE WIDTH  (x-DIR):  "<<setw(9)<<L<<"CM"<<endl;
			 outfile<<"          TEMPERATURE AT y=0 (j=1):"<<setw(11)<<TR[1]<<"K,  EMITTANCE  "<<setw(4)<<EPSR[1]<<endl;
			 outfile<<"          TEMPERATURE AT y=R (j=J):"<<setw(11)<<TR[2]<<"K,  EMITTANCE  "<<setw(4)<<EPSR[2]<<endl;
			 outfile<<"          TEMPERATURE AT x=0 (i=1):"<<setw(11)<<TX[1]<<"K,  EMITTANCE  "<<setw(4)<<EPSX[1]<<endl;
			 outfile<<"          TEMPERATURE AT x=L (i=I):"<<setw(11)<<TX[2]<<"K,  EMITTANCE  "<<setw(4)<<EPSX[2]<<endl;
			}
		else
			{
			 outfile<<"          GENERAL DATA"<<endl;
			 ...
			 outfile<<"          RECTANGLE HEIGHT (y-DIR):  "<<setw(9)<<R<<"CM"<<endl;
			 outfile<<"          RECTANGLE WIDTH  (x-DIR):  "<<setw(9)<<L<<"CM"<<endl;
			 outfile<<"          TEMPERATURE AT r=R(j=J): "<<setw(11)<<TR[2]<<"K,  EMITTANCE  "<<setw(4)<<EPSR[2]<<endl;
			 outfile<<"          TEMPERATURE AT x=0(i=1): "<<setw(11)<<TX[1]<<"K,  EMITTANCE  "<<setw(4)<<EPSX[1]<<endl;
			 outfile<<"          TEMPERATURE AT x=L(i=I): "<<setw(11)<<TX[2]<<"K,  EMITTANCE  "<<setw(4)<<EPSX[2]<<endl;
			}

		outfile<<" "<<endl;
		outfile<<"     MEDIUM TEMPERATURE TM (K)"<<endl;
		outfile<<" "<<endl;
		outfile<<"     J  ";
		for (J=1;J<=JJ;J=J+JPR)
		  {if (J==1) outfile<<setw(3)<<J;
			else outfile<<setw(6)<<J;}
		outfile<<" "<<endl;
		outfile<<"   I"<<endl;

		outfile<<setiosflags(ios::fixed | ios::showpoint)<<setprecision(0);

		for (I=1;I<=II;I=I+2)
		 {outfile<<setw(4)<<I<<"   ";
			for (J=1;J<=JJ;J=J+JPR)
				{outfile<<setw(6)<<TM[I][J];}
				 outfile<<" "<<endl;}

		outfile<<" "<<endl;
		outfile<<"     INCIDENT RADIATION G (W/SQCM)"<<endl;
		outfile<<" "<<endl;
		outfile<<"     J  ";
		for (J=1;J<=JJ;J=J+JPR)
		  {if (J==1) outfile<<setw(3)<<J;
		...
		outfile<<setiosflags(ios::fixed | ios::showpoint)<<setprecision(1);

		for (I=1;I<=II;I=I+2)
		 {outfile<<setw(4)<<I<<"   ";
			for (J=1;J<=JJ;J=J+JPR)
				{outfile<<setw(6)<<G[I][J];}
				 outfile<<" "<<endl;}

		outfile<<" "<<endl;
      outfile<<" "<<endl;
      outfile<<"     WALL FLUXES AT X=O AND X=L (W/SQCM)"<<endl;
      outfile<<" "<<endl;
      
      outfile<<"     J";
      for (J=1;J<=JJ;J=J+JPR)
			{if (J==1) outfile<<setw(5)<<J;
				else outfile<<setw(6)<<J;}
      outfile<<" "<<endl;
      outfile<<" "<<endl;
		...
		outfile<<"     Q";
      for (J=1;J<=JJ;J=J+JPR)
        {outfile<<setw(6)<<QX[1][J];}
      outfile<<" "<<endl;

      outfile<<"     Q";
      for (J=1;J<=JJ;J=J+JPR)
        {outfile<<setw(6)<<QX[2][J];}
      outfile<<" "<<endl;
      outfile<<" "<<endl;
      ...
      if(KK==0) 
        {
			outfile<<"     WALL FLUXES AT Y=O AND Y=R (W/SQCM)"<<endl;
			outfile<<" "<<endl;
			outfile<<"          I     QI      QR"<<endl;
			for (I=1;I<=II;I=I+1)
				{outfile<<setw(11)<<I<<setw(8)<<QR[1][I]<<setw(8)<<QR[2][I]<<endl;}
		  }
      else
        {
		  outfile<<"     RADIAL FLUXES TO CYLINDER WALL (W/SQCM)"<<endl;
		  outfile<<" "<<endl;
		  outfile<<"          I     QR"<<endl;
		  for (I=1;I<=II;I=I+1)
			  {outfile<<setw(11)<<I<<setw(8)<<QR[2][I]<<endl;}
		  }
