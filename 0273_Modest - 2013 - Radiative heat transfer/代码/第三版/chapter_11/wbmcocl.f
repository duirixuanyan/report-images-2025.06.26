...
C *** CARBON MONOXIDE ***
      ...
      DIMENSION ETA(1),DELTA(1),G(1),A0(1),B0(1),U(1),PHIE(2),PSIE(2)
      CHARACTER*12 BAND(2)
C
      WRITE(6,1)
    1 FORMAT('WBM FUNCTIONS FOR CO: please input temperature (in K)'/)
      READ(5,*) TEMP
      WRITE(6,2)TEMP
...
C
      ...
      G(1)=1.D0
C
C SCAN OVER THE 2 IMPORTANT BANDS
C
      DO 2000 IB=1,2
C
...
C *** 4.7MUM BAND ****
      IF(IB.EQ.1) THEN
            BAND(IB)='4.7 MUM BAND ' 
            DELTA(1)=1.D0
            ENDIF
C *** 2.35MUM BAND ****
      IF(IB.EQ.2) THEN
            BAND(IB)='2.35MUM BAND ' 
            DELTA(1)=2.D0
            A0(1)=2.D0
            ENDIF
C
