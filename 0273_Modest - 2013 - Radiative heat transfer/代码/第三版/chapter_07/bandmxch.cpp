...
void BANDAPP(int,double[],double[],double[][N+1],double[][N+1],double[][N+1],double[][N+1][N+1],int[],double[],double[]);
...
int main ()
    {
    double A[N+1],Bandwl[M+1],Fs[M+1][N+1][N+1],EPS[M+1][N+1],RHOs[M+1][N+1],HOs[M+1][N+1],T[N+1],q[N+1],L1,L2;
    int id[N+1],iclsd,i;
    ...
    HOs[1][1]=1203.5*bbfn(4.*5762);
    HOs[2][1]=1203.5-HOs[1][1];                   // solar irradiation including specular reflection from A2
    EPS[1][1]=0.8;
    RHOs[1][1]=0.0;
    EPS[2][1]=0.1;
    RHOs[2][1]= 0.0;
...
// Surface2 (left)
    ...
    HOs[1][2]=500.*bbfn(4.*5762);
    HOs[2][2]=500.-HOs[1][2];                     // direct solar irradiation
    EPS[1][2]=0.1;
    RHOs[1][2]=0.9;
    EPS[2][2]=0.8;
    RHOs[2][2]= 0.2;
...
// Solve system of equations
    BANDAPP(iclsd,A,Bandwl,EPS,RHOs,HOs,Fs,id,q,T);
// Output
...
//******************************************************************************
void BANDAPP(int iclsd,double A[],double Bandwl[],double EPS[][N+1],double RHOs[][N+1],double HOs[][N+1],double Fs[][N+1][N+1],int id[],double q[],double T[])
{
...
// T(N)   = vector containing surface temperatures (for all surfaces)
   ...
   double PIN[N+1],POUT[N+1],epsl[N+1],rhosl[N+1],hosl[N+1],FSl[N+1][N+1],Tmin,Tmax;
...
// Set local values for eps, rhos, hos, pin and Fs
       ...
       FSl[i][k]=Fs[j][i][k];
        ...
        GRAYDIFFSPEC(iclsd,A,epsl,rhosl,hosl,FSl,IDl,PIN,POUT);
