...
arg(2)=ll;
arg(3)=hh;     % a,b,c
Fs(1,3) = view(38,3,arg)+RHOs(2)*parlplates(ww,ww,2.*ww,ll,0.,ll,hh);  % Fs1-3=F1-3+rhos2*F1(2)-3
Fs(1,4) = Fs(1,2)+RHOs(2)*perpplates(ww,2.*ww,0.,hh,ll,0.,ll);         % Fs1-4=F1-4+rhos2*F1(2)-4
...
arg(2)=ll;
arg(3)=hh;    % h,l,w
Fs(2,3)=view(39,3,arg)+RHOs(1)*perpplates(hh,2.*hh,0.,ww,ll,0.,ll);  % Fs2-3=F2-3+rhos1*F1(1)-3

% Fs24
arg(1)=ll;
arg(2)=hh;
arg(3)=ww;     % a,b,c
Fs(2,4)=view(38,3,arg)+RHOs(1)*parlplates(hh,hh,2.*hh,ll,0.,ll,ww);  % Fs2-4=F2-4+rhos1*F2(1)-4
...
arg(2)=ll;
arg(3)=2.*hh;  % a,b,c
Fs(3,3)=RHOs(1)*(view(38,3,arg)+RHOs(2)*parlplates(ww,ww,2.*ww,ll,0.,ll,2.*hh));
...
arg(2)=ll;
arg(3)=2.*ww;  % a,b,c
Fs(4,4)=RHOs(2)*(view(38,3,arg)+RHOs(1)*parlplates(hh,hh,2.*hh,ll,0.,ll,2.*ww));
