...
!  ******************************************************************************
    DIMENSION ARG(10)
    CHARACTER*1 x
10  write(*,*) 'view factor number? (number in App. D or 98 for perpplates and 99 for parlplates)'
    read(*,*)  NO
    ...
    enddo
    IF(NO<1) THEN
        write(*,*) 'Illegal view factor number!'
        GOTO 10
    ELSEIF(NO<52) THEN
        F=VIEW(NO,NARG,ARG)
    ELSEIF(NO==98) THEN
        if(narg/=7) write(*,*) 'Illegal number of arguments for PERPPLATES!'
        X1=ARG(1)
        X2=ARG(2)
        Y1=ARG(3)
        Y2=ARG(4)
    ...
    ELSEIF(NO==99) THEN
        if(narg/=7) write(*,*) 'Illegal number of arguments for PARLPLATES!'
        X1=ARG(1)
        X2=ARG(2)
        X3=ARG(3)
        Y1=ARG(4)
