...
! from collimated light source, through purely scattering slab,
! using backward Monte Carlo
    USE DFLIB
    IMPLICIT NONE
    INTEGER :: i,N,in,idum=12345,numsmpl,numsmplhf,ns1=1
    REAL    :: L,sig,QT,DQ,x0,dx,dy,R,qd(50),thd,psi,xi,yi,zi
    REAL    :: sx,sy,sz,lbot,qdav,stddev,xe,ye,ze,sinth,ri,re,mu
    REAL    :: lscat,szdet,Ibndl,z1,z2,lpart,a,b,c,d,Qadj,stddevmax,var
    REAL, PARAMETER :: pi=3.1415927
    REAL, EXTERNAL :: RAN1
    open(unit=8,file='revmccs.dat',status='unknown')
    write(8,1)
    write(*,1)
...
!    
    ...
    Qadj=QT/(4.*pi*R*R)*dx*dy*(1.-szdet*szdet)
...
! check whether bundle leaves domain
          ...
          IF(ze>L) THEN
              ...
              GOTO 15
          ENDIF
! check what part of lscat is inside collimated beam
          ri=sqrt(xi*xi+yi*yi)
          re=sqrt(xe*xe+ye*ye)
          a=ri*ri-R*R
          b=sx*xi+sy*yi
...
! initial point is inside
             IF(re<R) THEN
                Ibndl=Ibndl+(exp(-sig*zi)-exp(-sig*ze))/sz
             ELSE
                IF(d<0.) GOTO 25           ! just a precaution; this cannot happen
                lpart=-(b-sqrt(d))/c       ! distance to collimated beam exit point
                z2=zi+lpart*sz
                Ibndl=Ibndl+(exp(-sig*zi)-exp(-sig*z2))/sz
             ENDIF
          ELSE
! initial point is outside
             ...
             IF(re<R) THEN
                 ...
                 z1=zi+lpart*sz
                 Ibndl=Ibndl+(exp(-sig*z1)-exp(-sig*ze))/sz
             ELSE
                 lpart=-(b+sqrt(d))/c      ! distance to collimated beam entry point
                 IF(lpart>lscat .or. lpart<0.) GOTO 25  ! never reaches collimated beam
                 z1=zi+lpart*sz
                 lpart=-(b-sqrt(d))/c      ! distance to collimated beam exit point
                 IF(lpart>lscat) lpart=lscat
                 z2=zi+lpart*sz
                 Ibndl=Ibndl+(exp(-sig*z1)-exp(-sig*z2))/sz
             ENDIF
          ENDIF
