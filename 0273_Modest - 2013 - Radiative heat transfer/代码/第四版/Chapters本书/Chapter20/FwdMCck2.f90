...
! Uses energy-partitioning ray tracing for absorption
    IMPLICIT NONE
    INTEGER :: i,N,in,idum=12345,numsmpl,numsmplhf,ns1=1
    REAL    :: L,kap,sig,QT,DQ,x0,dx,dy,R,qd(50),thd,ri,psi,xi,yi,zi,lscat,szdet
    REAL    :: sx,sy,sz,lbot,qdav,stddev,xe,ye,ze,sinth,DQlcl,stddevmax,var
    REAL, PARAMETER :: pi=3.145927
    REAL, EXTERNAL :: RAN1
    open(unit=8,file='fwdmck2.dat',status='unknown')
    write(8,1)
    write(*,1)
