...
! CONSTANT VALUES THROUGHOUT
      IMPLICIT NONE
      INTEGER,PARAMETER   :: II=41, JJ=21, KK=1, IRE=0
      INTEGER             :: I,J,LU,JPR
      DOUBLE PRECISION    :: QR(2,II),QX(2,JJ),EPSR(2),EPSX(2),TR(2),TX(2),TM(II,JJ),G(II,JJ),L,R
      DOUBLE PRECISION    :: SIG,A1(II,JJ),KT(II,JJ),ST(II,JJ),BT(II,JJ),SS(II,JJ),SR(2,II),SX(2,JJ)
!
! OUTPUT FILE
    OPEN(UNIT=9,FILE='P1_2Dsor.dat')
      LU=9
      SIG=5.670D-12     ! W/cm2K4
      TX(1)=2000.D0     ! ASSUME UNIFORM TEMPERATURE FOR ALL NODES
      TX(2)=0.D0        ! ASSUME UNIFORM TEMPERATURE FOR ALL NODES
      TR(1)=0.D0        ! ASSUME UNIFORM TEMPERATURE FOR ALL NODES
      TR(2)=0.D0        ! ASSUME UNIFORM TEMPERATURE FOR ALL NODES
      TM=0              ! ASSUME UNIFORM TEMPERATURE FOR ALL NODES
      R=10.D0           ! cm
      L=20.D0           ! cm
      EPSR=1.D0         ! BLACK LINER
      EPSX(1)=.5D0      ! BASE AT X=0
      EPSX(2)=1.D0      ! TOP AT X=L
      KT=1.D-1          ! cm-1; CONST THROUGHOUT
      ST=1.D-1          ! cm-1; CONST THROUGHOUT
      A1=0.D0           ! CONST THROUGHOUT
!
...
!
      DO 20 I=1,II
      DO 20 J=1,JJ
  20  SS(I,J)=4.D0*SIG*TM(I,J)**4   ! Temp-field (IRE=0) or source Q''' (IRE=1)
!
! ESTABLISHMENT OF BOUNDARY SOURCES IF PRESENT
!
      DO I=1,II
        SR(2,I)=4.D0*SIG*TR(2)**4
        SR(1,I)=4.D0*SIG*TR(1)**4
      ENDDO
      DO J=1,JJ
        SX(1,J)=4.D0*SIG*TX(1)**4
        SX(2,J)=4.D0*SIG*TX(2)**4
      ENDDO 
      
!
! CALCULATION OF INCIDENT RADIATION FIELD
!
      CALL P1SOR(II,JJ,KK,IRE,L,R,EPSX,EPSR,SX,SR,KT,ST,A1,SS,G,QX,QR)
!
! CALCULATE TEMPERATURE IF RADIATIVE EQUILIBRIUM (IRE=1)
!
      IF(IRE==1) TM=((SS/(KT+1.E-10)+G)/(4.D0*SIG))**.25
!
! OUTPUT
!
      IF(KK==0) THEN
        WRITE(LU,1200) R,L,TR(1),EPSR(1),TR(2),EPSR(2),TX(1),EPSX(1),TX(2),EPSX(2)
      ELSE
        WRITE(LU,1210) R,L,TR(2),EPSR(2),TX(1),EPSX(1),TX(2),EPSX(2)
      ENDIF
      WRITE(LU,1110)
      WRITE(LU,2200) (J,J=1,JJ,JPR)
      DO 140 I=1,II,2
 140  WRITE(LU,2510) I,(TM(I,J),J=1,JJ,JPR)
!
      WRITE(LU,1100)
      WRITE(LU,2200) (J,J=1,JJ,JPR)
      DO 150 I=1,II,2
 150  WRITE(LU,2500) I,(G(I,J),J=1,JJ,JPR)
!
      WRITE(LU,3000) (J,J=1,JJ,JPR)
      WRITE(LU,3200) (QX(1,J),J=1,JJ,JPR)
      WRITE(LU,3200) (QX(2,J),J=1,JJ,JPR)
      IF(KK==0) THEN
        WRITE(LU,3100) (I,QR(1,I),QR(2,I),I=1,II)
      ELSE
        WRITE(LU,3110) (I,QR(2,I),I=1,II)
      ENDIF
      
1100  FORMAT(/5X,'INCIDENT RADIATION G (W/SQCM)'/)
1110  FORMAT(5X,'MEDIUM TEMPERATURE TM (K)'/)
1200  FORMAT(10X,'GENERAL DATA'/10X,'************'// &
         10X,'RECTANGLE HEIGHT (y-DIR):  ',F6.2,'CM'/     &
         10X,'RECTANGLE WIDTH  (x-DIR):  ',F6.2,'CM'/     &
         10X,'TEMPERATURE AT y=0 (j=1):',F8.2,'K,  EMITTANCE  ',F4.2/     &
         10X,'TEMPERATURE AT y=R (j=J):',F8.2,'K,  EMITTANCE  ',F4.2/     &
         10X,'TEMPERATURE AT x=0 (i=1):',F8.2,'K,  EMITTANCE  ',F4.2/     &
         10X,'TEMPERATURE AT x=L (i=I):',F8.2,'K,  EMITTANCE  ',F4.2/)
1210  FORMAT(10X,'GENERAL DATA'/10X,'************'// &
         10X,'CYLINDER RADIUS (R-DIR):   ',F6.2/     &
         10X,'CYLINDER LENGTH (X-DIR):   ',F6.2/     &
         10X,'TEMPERATURE AT r=R(j=J): ',F8.2,'K,  EMITTANCE  ',F4.2/     &
         10X,'TEMPERATURE AT x=0(i=1): ',F8.2,'K,  EMITTANCE  ',F4.2/     &
         10X,'TEMPERATURE AT x=L(i=I): ',F8.2,'K,  EMITTANCE  ',F4.2/)
2200  FORMAT(4X,2H\J,I3,10I6/3X,1HI)
2300  FORMAT(I4,11F6.0)
2500  FORMAT(I4,11F6.1)
2510  FORMAT(I4,11F6.0)
3000  FORMAT(//5X,'WALL FLUXES AT X=O AND X=L (W/SQCM)'// &
               5X,1HJ,I5,10I6/)
3200  FORMAT(5X,1HQ,11F6.1)
3100  FORMAT(//5X,'WALL FLUXES AT Y=O AND Y=R (W/SQCM)'// &
              10X,'I     QI      QR'//(8X,I3,2F8.1))
3110  FORMAT(//5X,'RADIAL FLUXES TO CYLINDER WALL (W/SQCM)'// &
              10X,'I     QR'//(8X,I3,F8.1))
      STOP
      END
