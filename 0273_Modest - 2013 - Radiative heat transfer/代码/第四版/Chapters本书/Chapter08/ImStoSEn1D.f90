...
!      Uses SEMI-IMPLICIT coupling (omega = 1 set below)
...
! Files to print
  OPEN(unit=10,file="ImStoSEn1D.dat",status="unknown") ! T and Flux
  OPEN(unit=20,file="ImStoSEn1D_residual.dat",status="unknown") ! Residual
...
!   Semi-implicit treatment
    amat(1,1) = amat(1,1) + 4.0d0*sigma*emis(1+3)*(tbc(1+3)**3)*Li
    rmat(1) = rmat(1) + 4.0d0*sigma*emis(1+3)*(tbc(1+3)**4)*Li
    DO i = 2,nx-1
...
!    Semi-implicit treatment
      amat(i,i) = amat(i,i) + 4.0d0*sigma*emis(i+3)*(tbc(i+3)**3)*Li
      rmat(i) = rmat(i) + 4.0d0*sigma*emis(i+3)*(tbc(i+3)**4)*Li
    ENDDO
    amat(nx,nx) = ho*(Li+Ls) + kc*Ls/Li
    amat(nx,nx-1) = -kc*Ls/Li
    rmat(nx) = ho*t_inf*(Li+Ls)-qrad(nx+3)*Li
!   Semi-implicit treatment
    amat(nx,nx) = amat(nx,nx) + 4.0d0*sigma*emis(nx+3)*(tbc(nx+3)**3)*Li
    rmat(nx) = rmat(nx) + 4.0d0*sigma*emis(nx+3)*(tbc(nx+3)**4)*Li
